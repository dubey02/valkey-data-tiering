...
/* ---------------------------------------------------------------------------
 * Fork read snapshot (the fallback strategy)
 *
 * Lives here rather than in ext_storage.c because "snapshot" is one concern with
 * two implementations, and splitting them across files meant a reader had to
 * know which strategy they were chasing before they knew which file to open.
 * ext_snapshot.c now owns both: the streaming cut above, and the fork read
 * fallback below.
 *
 * The fallback quiesces tiering IO, parks the storage IO thread and pauses
 * on-flash GC before fork(), so the child inherits consistent backend state and
 * stable flash offsets, then materializes each tiered value with a synchronous
 * fork-child-safe read. It pays a random read pass over the live flash set and
 * holds GC paused for the child's lifetime, which is exactly what the streaming
 * path exists to avoid -- but it is the only option for a backend that cannot
 * stream.
 *
 * The extStorage* prefix is kept: these are part of the published contract and
 * renaming them would churn call sites for no behavioural gain.
 * ---------------------------------------------------------------------------*/
