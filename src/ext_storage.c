 ...
 *
 * Connects the valkey server to a storage engine. It holds the serialization
 * callbacks, opens the configured storage engine at startup, and reports its
 * state through INFO. */
 ...
 * Serialization
 *
 * The storage engine may call these from its own IO thread. Value
 * deserialization reads valkey server config through rdbLoadObject, so it takes
 * ext_storage_loading_mutex to stay consistent with a concurrent config change.
 * ---------------------------------------------------------------------------*/

static int extStorageSerializeKey(void *key, char **out) {
    sds s = key;
    *out = s;
    return (int)sdslen(s);
}
...
static void *extStorageDeserializeKey(const char *bytes, int len) {
    ...
    return sdsnewlen(bytes, (size_t)len);
}
...
static void *extStorageDeserializeValue(const char *bytes, int len) {
    ...
    robj *o = NULL;
    if (type != -1) {
        /* rdbLoadObject reads valkey server config, so guard it against a concurrent
         * config change. See ext_storage_loading_mutex. */
        if (extStorageTryAcquireLoadingLock()) {
            o = rdbLoadObject(type, &payload, NULL, -1, NULL, RDBFLAGS_NONE, 0);
...
void extStorageInit(void) {
    ...
    /* Resolve the storage engine by explicit precedence. A module registered
     * its storage engine during module load, and it wins if its name matches.
     * Otherwise the built-in mock is selected by name. Anything else is a
     * storage engine that was configured but never provided. */
    ...
    /* storageRegisterEngine already checked a module storage engine's version,
     * but a built-in storage engine is selected by name and skips that path, so
     * verify the resolved storage engine here. The config is immutable, so a
     * mismatch is fatal. */
